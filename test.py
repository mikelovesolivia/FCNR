...
def _evaluation_step(left, right, pl, pr, pos, model, eval_logger, CFG, left_name, right_name, epoch):
    ...
    if not os.path.exists('./results/'+tl+"/"):
        os.makedirs('./results/'+tl+"/")
    if not os.path.exists('./results/'+tr+"/"):
        os.makedirs('./results/'+tr+"/")

    if CFG.log_images and epoch==CFG.epochs-1:
        ...
        unloader(pred_left.cpu().squeeze(0)).save('./results/'+tl+'/'+left_name+"_left.png")
        unloader(pred_right.cpu().squeeze(0)).save('./results/'+tr+'/'+right_name+"_right.png")
